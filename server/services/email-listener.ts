...
import { sendEmail, sendInventoryUpdateEmail } from './email';
import fs from 'fs';
...
function validateEmail(emailData: { from: string; subject: string }) {
  ...
  const hasRequiredKeyword = defaultConfig.requiredSubjectKeywords.some(keyword =>
    subjectLower.includes(keyword.toLowerCase())
  );
  
  if (!hasRequiredKeyword) {
    console.log(`Subject "${emailData.subject}" does not contain required keywords`);
    return false;
  }
...
export function initializeEmailListener(config?: Partial<EmailListenerConfig>) {
  ...
  console.log(`Email listener would check for new emails every ${mergedConfig.checkIntervalMinutes} minutes`);
