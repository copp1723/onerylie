 ...
 */
export async function selectPromptVariant(context: ABTestContext): Promise<PromptVariant | null> {
  try {
    ...
    if (context.experimentId && context.conversationId) {
      ...
      const existingMetric = await db.select()
        .from(promptMetrics)
        .where(
          eq(promptMetrics.conversationId, context.conversationId)
        )
        .limit(1);
    ...
    if (activeExperiments.length === 0) {
      ...
      const [anyVariant] = await db.select()
        .from(promptVariants)
        .where(
          and(
            eq(promptVariants.dealershipId, context.dealershipId),
    ...
    const selectedExperiment = activeExperiments[Math.floor(Math.random() * activeExperiments.length)];
    
    // Get all variants for this experiment with their traffic allocation
    const variantsWithAllocation = await db.select({
    ...
    const totalAllocation = variantsWithAllocation.reduce((sum, { experiment }) => 
      sum + (experiment.trafficAllocation || 50), 0);
    ...
    if (context.conversationId && selectedVariant) {
      recordExperimentAssignment(selectedExperiment.id, selectedVariant.id, context.conversationId);
    }
...
export async function createExperiment(
  ...
  try {
    ...
    const totalAllocation = data.variantAssignments.reduce(
      (sum, assignment) => sum + assignment.trafficAllocation, 0
    );
...
export async function getExperimentResults(experimentId: number) {
  try {
    ...
    const variantsInExperiment = await db.select({
      experiment: experimentVariants,
      variant: promptVariants
    })
    .from(experimentVariants)
    ...
    for (const { variant, experiment: expVariant } of variantsInExperiment) {
      ...
      const avgResponseTime = metrics.reduce((sum, m) => sum + (m.responseTime || 0), 0) / 
        (metrics.filter(m => m.responseTime !== null).length || 1);
      const avgTokensUsed = metrics.reduce((sum, m) => sum + (m.tokensUsed || 0), 0) / 
        (metrics.filter(m => m.tokensUsed !== null).length || 1);
      const escalationRate = metrics.filter(m => m.wasEscalated).length / (totalInteractions || 1);
      const successRate = metrics.filter(m => m.wasSuccessful).length / 
      ...
      const avgCustomerRating = ratedMetrics.length > 0 
        ? ratedMetrics.reduce((sum, m) => sum + (m.customerRating || 0), 0) / ratedMetrics.length
        : null;
 ...
 */
export async function getPromptVariants(dealershipId: number, includeInactive = false) {
  try {
    let query = db.select()
      .from(promptVariants)
 ...
 */
export async function endExperiment(experimentId: number, conclusionNotes?: string) {
  try {
    await db.update(promptExperiments)
      .set({
        isActive: false,
        endDate: new Date(),
        conclusionNotes
